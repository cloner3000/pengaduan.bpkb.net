...
class Register_user_model extends CI_Model{
    ...
    function get(){
      return $this->db->query("select * from ticket_user where type_id = '1'");
    }
    ...
    function save($data){
      if($data['uuid'] != ''){
        ...
        if($query->num_rows() > 0 ){
          return setpesan('warning',"email sudah terdaftar. jika anda lupa password silahkan recovery password pada menu login. terima kasih");
        }
    ...
    function dataajax(){
      $this->datatables
        ->select('ticket_user.uuid,email,full_name,phone,gender,identity_no,address,
          type_identity,actived,rt,rw,village,sub,province')
        ->from("ticket_user")
    ...
    function detail($id){
      $this->db->select('ticket_user.uuid,email,full_name,phone,gender,identity_no,address,
